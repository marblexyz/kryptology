...
func TestLcmBigPrimes(t *testing.T) {
	...
	y.SetString(
		"",
		10,
	)
	expected := new(big.Int)
	expected.SetString(
		"",
		10,
	)
	...
	y3 := new(big.Int)
	x3.SetString(
		"",
		10,
	)
	y3.SetString(
		"",
		10,
	)
	expected2 := new(big.Int)
	expected2.SetString(
		"",
		10,
	)
	testArgs := []lcmTest{
		{
...
func TestAdd(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			pk, tt.B10("11659564086467828628"), tt.B10("57089538512338875950"),
			tt.B10("665639132951488346363609789106750696600"),
		},
	}
	// All the tests!
	for _, test := range tests {
